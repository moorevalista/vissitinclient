...
export default function KonfirmasiPembayaran(props) {
  ...
  async function onProses() {
    ...
    if(success.status === true) {
      if(success.res.responseCode === '000') {
        ...
        // if(success.res.data !== undefined && paymentMethod === 'online') {
        //   setLoadingSave(false);
        //   alert(success.res.data.link_url.replace('flip.id/', formValidation.flipPaymentUrl));
      ...
      await formValidation.showError('Terjadi kesalahan...');
      setLoadingSave(false);
    }
    //setLoadingSave(false);
  }
  ...
  //untuk copy VA
  const copyVa = e => {
    formValidation.showError('VA berhasil di salin');
